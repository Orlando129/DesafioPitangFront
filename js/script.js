...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Selecione os elementos do DOM que você deseja atualizar
    const nomeCliente = document.getElementById("nomeCliente");
    const cnpjCliente = document.getElementById("cnpjCliente");
    const creditoCliente = document.getElementById("creditoCliente");
    const emailCliente = document.getElementById("emailCliente");

    // Obtenha os valores dos atributos de dados do cliente da URL
    const urlParams = new URLSearchParams(window.location.search);
    const clienteNome = urlParams.get("cliente-nome");
    const clienteCnpj = urlParams.get("cliente-cnpj");
    const clienteCredito = urlParams.get("cliente-credito");
    const clienteEmail = urlParams.get("cliente-email");


    // Preencha os campos com os valores obtidos
    if (nomeCliente) {
        nomeCliente.textContent = clienteNome;
    }

    if (cnpjCliente) {
        cnpjCliente.textContent = clienteCnpj;
    }

    if (creditoCliente) {
        creditoCliente.textContent = clienteCredito;
    }
...
function carregarPaginaVisualizacaoCliente(clienteId, clienteNome, clienteCnpj, clienteCredito, clienteEmail) {
    ...
    pageContent.innerHTML = `
        ...
        <p><strong>CNPJ:</strong> ${clienteCnpj}</p>
        <p><strong>Crédito:</strong> ${clienteCredito}</p>
        <p><strong>Email:</strong> ${clienteEmail}</p>
    `;
}

// Evento de clique nos links da página de "Listagem de Clientes"
document.querySelectorAll("#listaClientes a").forEach((link) => {
    link.addEventListener("click", function (event) {
        event.preventDefault();
        const clienteId = link.getAttribute("data-cliente-id");
        const clienteNome = link.getAttribute("data-cliente-nome");
        const clienteCnpj = link.getAttribute("data-cliente-cnpj");
        const clienteCredito = link.getAttribute("data-cliente-credito");
        const clienteEmail = link.getAttribute("data-cliente-email");
        carregarPaginaVisualizacaoCliente(clienteId, clienteNome, clienteCnpj, clienteCredito, clienteEmail);
    });
});
